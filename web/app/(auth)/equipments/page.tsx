...
export default function EquipmentPage() {
  ...
  const { role, isCoordinator, loading: roleLoading } = useUserRole()
  ...
  // Fetch equipment and categories from API
  const fetchEquipment = async () => {
    try {
      const data = await apiRequest<any>('/equipment')
      setEquipment(data.content || data)
      setFilteredEquipment(data.content || data)
    } catch (error) {
  ...
  const fetchCategories = async () => {
    try {
      const data = await apiRequest<any>('/equipment/categories')
      setCategories(data)
    } catch (error) {
      ...
      <div className="min-h-screen flex flex-col" style={{ marginLeft: '280px' }}>
        ...
        <main className="flex-1 p-8 pt-6 overflow-y-auto">
          ...
          <div className="flex justify-between items-center mb-6">
            ...
            {/* Only show Add Equipment button for Coordinators and Admins */}
            {isCoordinator && (
              <button 
              ...
              {filteredEquipment.map((item) => (
                ...
                >
                  <div className="flex justify-between items-start mb-3">
                    <div className="w-12 h-12 rounded-xl bg-linear-to-br from-sky-500/20 to-cyan-500/20 flex items-center justify-center">
                      <div className="w-6 h-6 rounded-lg bg-sky-500/30 flex items-center justify-center text-sky-500 font-bold">
                        {item.name.charAt(0)}
                      </div>
                    </div>
                    {/* Only show edit button for coordinators */}
                    {isCoordinator && (
            ...
            <div className="space-y-3">
              ...
              {filteredEquipment.map((item) => (
                ...
                >
                  ...
                  <div className="col-span-3 text-slate-400 text-sm">{item.category}</div>
